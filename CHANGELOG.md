...
- Added `DESTINATION_NOT_SERVICED` to `CheckoutErrorCode` and `CheckoutCreateFromOrderUnavailableVariantErrorCode`. Checkout mutations that validate line quantities (`checkoutCreate`, `checkoutLinesAdd`, `checkoutLinesUpdate`, `checkoutShippingAddressUpdate`, `checkoutCreateFromOrder`) now return this code, with the message `We do not ship to {country}. {variant} cannot be delivered to this address.`, when no warehouse in the checkout's channel can ship to the given address country — previously reported as `INSUFFICIENT_STOCK` with `Only 0 remaining in stock.` even though stock was untouched. `INSUFFICIENT_STOCK` still covers the case where a warehouse can ship to the destination but holds too few units, including stock held only in a click-and-collect warehouse. Clients matching on `INSUFFICIENT_STOCK` to detect an undeliverable address should handle the new code.
...
- Fix `TransactionInitialize` and `TransactionProcess` allowing a payment app to be charged against a checkout that could never actually complete (e.g. no shipping method, no billing address, no lines) — this now raises a validation error before any payment app is asked to charge, instead of only failing later at `checkoutComplete` with the charge already taken.
- `TransactionInitialize`/`TransactionProcess` now store a snapshot of the checkout's contents (lines, shipping method, total) in the transaction's private metadata (`checkout_payment_snapshot_initialize`/`checkout_payment_snapshot_process`) at the moment payment is attempted, so a later mismatch between the amount charged and the checkout's (possibly since-changed) total can be diagnosed against what was actually being paid for.
- `create_order_from_checkout` (used by both `checkoutComplete` and `orderCreateFromCheckout`) now compares that snapshot against the checkout's current contents before creating the order, for every completion path — a storefront calling `checkoutComplete` directly, a payment app's own reconciliation, or the automatic-checkout-completion background task. If the actual line items changed since payment, order creation is now refused with a new `CONTENT_CHANGED_AFTER_PAYMENT` error code (`CheckoutErrorCode`/`OrderCreateFromCheckoutErrorCode`) instead of silently completing for something other than what was paid for. Shipping/tax-only drift doesn't block completion (common and often benign) but is recorded on the resulting order's private metadata (`payment_reconciliation_diff`) so it's visible to whoever investigates a payment discrepancy later. No-ops (backward compatible) for transactions created before this feature existed, and for orders created without going through `TransactionInitialize`/`TransactionProcess` at all.
