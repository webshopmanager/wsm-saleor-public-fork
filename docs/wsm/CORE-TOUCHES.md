...
Sections 1 to 6 and the two monkey patches are this branch's own work and touch
two core files. Section 7 is the six WSM patches that already existed before the
fork, merged in from `wsm/bakeoff-rebase-probe`; they are the reason the guard
test's allow-list is longer than two entries.
