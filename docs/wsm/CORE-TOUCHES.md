...
## 2. `saleor/settings.py`, +2 lines (U3, 2026-09-08)
...
## 3. `saleor/urls.py`, +3 lines (U2 and U3 together, 2026-09-08)

```python
from django.urls import include, path, re_path   # was: from django.urls import re_path
...
...
**Shared touch.** U2 (Compose endpoints) and U3 (Dealer endpoints) each need the
fork's URLs mounted, and both branches add these same three lines. Expect a
trivial conflict when the second one merges: keep one copy.
...
that file, which upstream does not own, rather than another core touch. The
prefix `/wsm/` is the fork's whole URL surface, so an upstream route can never
collide with ours.

Removal cost: delete the three lines. Nothing in core resolves a `wsm-` route name.
